/* alerts.js — the dashboard noticing something happened while you were looking
 * at it.
 *
 * The page auto-refreshes every 10 minutes while it is visible (app.js), and
 * this is what makes that worth doing: the difference between one ping report
 * and the next is announced rather than silently redrawn. Three events, in
 * ascending order of how much they matter:
 *
 *   visitors   someone opened the app          soft blip, no card
 *   downloads  a first run the counter had     two-note chime, confetti falling
 *              never seen (a new install)      from the top, a card naming the
 *                                              store(s) it came from
 *   sales      a subscribe ping                fanfare, confetti from the top
 *                                              AND the bottom, a card naming
 *                                              the store(s) that paid
 *
 ...
 *   the oldest day drops out of it as the calendar turns and a total can fall
 ...
 * - **The first report of a session is a baseline, not news.** Nothing fires
 *   until there is a previous snapshot to compare against, or every sign-in
...
(function () {
  ...
  function snapshot(report) {
    var out = { opens: 0, downloads: 0, sales: 0, downloadsBy: {}, salesBy: {} };
   ...
   *
   * A single arrival is one burst and about a second and a half of falling.
   * More than one runs in WAVES instead — a wave per item, 320ms apart, capped
   * at `MAX_WAVES` — so three downloads at once is a few seconds of confetti
   * rather than the same one-second puff three times over, which is not
   * distinguishable from one. The cap is what stops a backfill of fifty
   * arriving pings burying the dashboard for half a minute.
   */
  var WAVE_MS = 320;
  var MAX_WAVES = 8;
  var waveTimer = 0;
  ...
  function celebrate(kind, count) {
    ...
    var waves = Math.max(1, Math.min(MAX_WAVES, Math.round(count) || 1));
    var fired = 0;
    var shoot = function () {
      if (kind === 'sale') burst(45); else rain(90);
    ...
    if (waves > 1) {
      ...
      waveTimer = window.setInterval(shoot, WAVE_MS);
    }
  }
  ...
  function announce(d) {
    if (d.sales > 0) {
      card('sale', d.sales + ' new ' + plural(d.sales, 'sale') + '!',
        storeLine(d.salesBy));
      play(SALE);
      celebrate('sale', d.sales);
    }
    if (d.downloads > 0) {
      card('download', d.downloads + ' new ' + plural(d.downloads, 'download') + '!',
        storeLine(d.downloadsBy));
   ...
   * announced, or null when there was nothing to say (including the first call
   * of the session, which only seeds the baseline).
   */
