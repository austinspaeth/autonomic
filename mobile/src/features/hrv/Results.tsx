...
export function HrvResults({ rr, segmentStarts, hrSamples, sdnnSamples, config, durationSec, startedAtMs, watchFallback, controls }: {
  ...
  const save = async () => {
    ...
    // And the daily counter beside it: this install measured today, and with
    // which sensor. A no-op on every reading after the first one of the
    // Eastern day — so the letter names the day's FIRST reading, not the day.
    pingHrvReading(config.source);
