...
export const HRV_HELP: Record<string, string> = {
  ...
  balance: 'PNS and SNS are the two branches of your autonomic nervous system, scored like a z-score where zero is the population average. PNS is your rest-and-recover (vagal) side, built from mean RR, RMSSD and SD1, and higher is better. SNS is your fight-or-flight side, built from heart rate, the stress index and RMSSD, and here lower is better, so a negative value means calm. They move like a see-saw: when you are recovered PNS sits above SNS, and under load SNS climbs above PNS. The chart fills green when PNS leads, amber where the lines cross, and red when SNS takes over.',
