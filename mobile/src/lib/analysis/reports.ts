...
export function entryCount(days: DaysMap, keys: string[]): number {
  let n = 0;
  keys.forEach((k) => { const d = days[k]; if (!d) return; n += (d.readings || []).length + (d.activities || []).length + (d.meds || []).length + (d.symptoms || []).length; n += ((d.food && d.food.meals) || []).length; if (d.food && d.food.triggers) n += Object.values(d.food.triggers).filter((v) => v > 0).length; n += ((d.digestion && d.digestion.movements) || []).length; if (d.sleep && (d.sleep.bed || d.sleep.wake)) n += 1; });
  return n;
}
