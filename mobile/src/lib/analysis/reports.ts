...
function secHRV(days: DaysMap, keys: string[]) {
  return orNone(eachEntry(days, keys, (d, k) => {
    ...
    (d.readings || []).forEach((r) => {
      if (r.type === 'hrv') out.push(`${stamp(k, r.time as string)} Type: ${r.source === 'watch' ? 'Apple Watch' : r.source === 'health' ? 'Imported' : 'Unstructured'} | HR: ${rv(r, 'avgHr') ?? '-'} | RMSSD: ${rv(r, 'rmssd') ?? '-'} | pNN50: ${rv(r, 'pnn50') ?? '-'} | SDNN: ${rv(r, 'sdnn') ?? '-'} | PNS: ${rv(r, 'pns') ?? '-'} | SNS: ${rv(r, 'sns') ?? '-'} | Stress: ${rv(r, 'stressIndex') ?? '-'} | VLF: ${rv(r, 'vlowPower') ?? '-'} | LF: ${rv(r, 'lowPower') ?? '-'} | HF: ${rv(r, 'highPower') ?? '-'}${noteSuffix(r)}`);
...
export const REPORT_CARDS: ReportCard[] = [
  ...
  { id: 'doctor', icon: 'clipboard', title: 'Medical Summary For Doctor', desc: 'A structured summary to help you start a conversation with your doctor.', sections: ['hrv', 'bp', 'rhr', 'sleep', 'symptoms', 'orthostatic', 'meds', 'supplements', 'scores'], focus: 'Generate a structured medical summary suitable for sharing with providers.', instructions: 'Summarize recent metrics/trends, persisting symptoms, and notable concerns from the data, plus questions for a physician. Professional tone; do not assert diagnoses not present in the data.' },
];
